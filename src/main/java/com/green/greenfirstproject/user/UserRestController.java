...
public class UserRestController {
    ...
    public Result emailCheck(@RequestParam String email)
    {
        try {
            ...
            if (user != null) return ResultError.builder().code(-2).msg("이미 가입된 이메일 입니다.").build();
    ...
    public Result verifyEmailToken(@RequestParam String token)
    {
        try {
            service.checkEmailToken(token) ;
        } catch (NullPointerException e) {
            return ResultError.builder().code(-2).msg("토큰 정보를 찾을수 없습니다.").build();
        } catch (TimeoutException e) {
            return ResultError.builder().code(-3).msg("토큰의 유효기간이 초과했습니다.(30분)").build();
        } catch (Exception e) {
            log.error("An error occurred: ", e);
            return ResultError.builder().build();
        }
    ...
    public Result postUser(@RequestBody UserInsertDto data)
    {
        ...
        if (!data.getId().matches(regex))
            return ResultError.builder().code(-2).msg("아이디 검증에 실패하였습니다.").build();
        ...
        try {
            ...
            return ResultError.builder().code(-5).msg("토큰 정보를 찾을수 없습니다.").build();
        } catch (TimeoutException e) {
            return ResultError.builder().code(-6).msg("토큰의 유효기간이 초과했습니다.(30분)").build();
        } catch (Exception e) {
            log.error("An error occurred: ", e);
            return ResultError.builder().build();
        }
        ...
        try {
            ...
            return ResultError.builder().code(-7).msg("중복된 이메일입니다.").build();
        } catch (Exception e) {
            log.error("An error occurred: ", e);
            return ResultError.builder().build();
        }
    ...
    public Result deleteUser(@RequestParam Long seq)
    {
        ...
        if (data == null) return ResultError.builder().code(-2).msg("세션 정보를 확인해 주세요.").build();
...
//
    ...
    {
        //유효성 검증
        if (service.duplicatedData(data.getName(),2)) return ResultError.builder().code(-3).msg("중복된 닉네임").build();
        PrincipalDetail principal = PrincipalUtil.getPrincipal() ;
        if (principal == null) return ResultError.builder().code(-2).msg("세션 정보를 확인해 주세요.").build();
    ...
    {
        if (!(type == 1 || type == 2)) return ResultError.builder().code(-2).msg("타입값 유효성 체크 실패").build();
        try {
            boolean data = service.duplicatedData(str,type) ;
            return ResultDto.<Boolean>builder().code(data ? 2 : 1).msg(data ? "이미 존재하는 문자열 입니다." : "존재하지 않는 문자열 입니다.").data(data).build();
        } catch (Exception e) {
            log.error("An error occurred: ", e);
            return ResultError.builder().build();
        }
